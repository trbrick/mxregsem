...
summary(lav.out)
cv.out <- cv_regsem(lav.out,pars_pen="loadings",metric = "EBIC")
cv.out
```
...
```{r}
omxCheckCloseEnough(cv.out$parameters[19,1:8], coef(regFit)[1:8], .002) # basically the same -- penalty on different scales
