...
mimicModel <- mxModel("MIMIC", type="RAM", 
                      ...
                      mxPath(from="one", to=manifests, values=c(rep(5, length(indicators)), rep(1, length(covariates)))),
...
```{r}
regMIMIC <- mxTryHard(regularizeMxModel(mimicModel, paste0("reg", 1:length(covariates)), penalty_value = 50, penalty_function = "lasso"))
```
...
```{r}
omxCheckCloseEnough(mimic.cvout$parameters[19,matches], estimates[matches], .05) # First approximation.
```
