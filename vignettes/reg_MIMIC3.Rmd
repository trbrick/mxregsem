...
ramCov <- mxModel(ramCov,                   # Regularization
                  mxRegularizeLASSO(regLabels, "LASSO", lambda=0, lambda.min=0, lambda.step=.04, lambda.max=5)
)
```

Let's run and plot, just like above.
```{r}
ramCovFit <- mxPenaltySearchExternal(ramCov, ebicGamma = 1, epsilon = .05, returnConstrained = TRUE)
summary(ramCovFit)
plotReg(ramCovFit)
```
